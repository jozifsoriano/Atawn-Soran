...
class Casino(commands.Cog):
    ...
    async def rps(self, ctx, choice, bet:int):
        ...
        if choice in choices:
            ...
            embed = discord.Embed(title='**JANKENPON**', description=f'CPU CHOSE *{cpu_choice}*', color=0x4432a8)
            if choice == cpu_choice:
                embed.add_field(name="It's a tie!", value=f"No money lost. ",inline = True)
            elif choice == 'rock' and cpu_choice == 'paper':
                embed.add_field(name="Fat L boi.", value=f"You lose {bet}. ",inline = True)
                payout_mult = 1
                winflag = 0
            elif choice == 'rock' and cpu_choice == 'scissors':
                embed.add_field(name="DUBS ONLY.", value=f"You win {2*bet} ",inline = True)
                payout_mult = 1
                winflag = 1
                ...
                payout_mult = 1
                winflag = 1
            final_balance = self.process_bet(str(ctx.author), bet, payout_mult, winflag)
            embed.add_field(name = f"{ctx.author}, your final balance is ",value=f"{self.read_cash(str(ctx.author))}.")
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title='**JANKENPON**', description=f'ERROR', color=0x4432a8)
            if flag == -1:
                embed.add_field(name=choice, value="Get yo money", inline = True)
            else:
                embed.add_field(name="Not a valid choice.",value= f"Choose {choices}. ", inline = True)
            await ctx.send(embed=embed)
    ...
    async def table(self, ctx):
        ...
        embed.add_field(name="parity(1:1)", value="[p1] for odd/[p2] for even", inline = True)
        embed.add_field(name="half(1:1)", value="[h1] for 1-18/[h2] for 19-36", inline = True)
        embed.add_field(name="column(2:1)", value="[c1]for 1st column,[c2]for 2nd column,[c3]for 3rd column", inline = True)
        embed.add_field(name="dozen(2:1)", value="[d1] for 1st 12,[d2] for 2nd 12,[d3] for 3rd 12]", inline = True)
    ...
    #number - 35to1 - [0-35]
        ...
        embed = discord.Embed(title="**ROULETTE**", description=f"Can I see your ID?", color =0xf538ff)
        ...
        if bet_choice in colors:
            ...
            payout_mult = 1
            bet_type = 'color'
            embed.add_field(name = bet_choice,value=f"{payout_mult}:1",inline=False)
            ...
            if flag == -1:
                embed.add_field(name=choice, value="Get yo money", inline = False)
            else:
                embed.add_field(title = f'ERROR, {ctx.author}; Bet choice invalid.', value = 'Use format [bet_type][bet_choice]')
            await ctx.send(embed=embed)
        if flag != -1:
            roll = random.randint(0,37)
            print(f'roll: {roll}')
            embed.add_field(name = '**WINNING NUMBER:**',value=f"**{roll}**",inline=False)
